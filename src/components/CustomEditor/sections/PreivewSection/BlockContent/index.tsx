...
const Img = ({ url, caption }) => {
  return (
    <>
      ...
      {caption.length ? (
        <p className='w-full text-center my-3 md:mb-12 text-base text-darkGrey'>{caption}</p>
      ) : (
        ''
      )}
    </>
...
const BlockContent = ({ block }) => {
  ...
  if (type === "header") {
    ...
    if (data.level === 4) {
      return <h4 className='text-xl font-bold' dangerouslySetInnerHTML={{ __html: data.text }} />;
    }
    if (data.level === 5) {
      return <h5 className='text-lg font-bold' dangerouslySetInnerHTML={{ __html: data.text }} />;
    }
    if (data.level === 6) {
      return <h6 className='text-md font-bold' dangerouslySetInnerHTML={{ __html: data.text }} />;
    }
  }
  if (type === 'image') {
